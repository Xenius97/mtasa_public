...
addEventHandler("onResourceStart", resourceRoot, function()
	...
	for k, v in ipairs(vehicleIDS) do
		table.insert(thread, v)
	end
	startNewWork(thread, loadVehicle, 250, 3)
end)

local defX, defY, defZ = 1568.2779541016, 1556.755859375, 10.8203125
local x, y, z = defX, defY, defZ
local count = 0
function loadVehicle(id, this, all)
	local veh = createVehicle(id, x, y, z, 0, 0, 90)
	if veh then
		if getVehicleType(veh) == "Automobile" or getVehicleType(veh) == "Bike" or getVehicleType(veh) == "Quad" then
